...
def _map_gender_detector(label: str) -> str:
    value = (label or "").strip().lower() if isinstance(label, str) else ""
...
def _build_gender_detector():
    ...
    primary = gender.Detector(case_sensitive=False)
    fallback = None
    try:
        from gender_detector.gender_detector import GenderDetector

        fallback = GenderDetector("uk")
    except Exception as exc:  # noqa: BLE001
        logging.getLogger(__name__).warning("gender-detector fallback unavailable: %s", exc)
    return primary, fallback


def _infer_gender_with_fallback(first: str, detector) -> str:
    primary, fallback = detector
    gender_label = _map_gender(primary.get_gender(first))
    if gender_label != "unknown" or fallback is None:
        return gender_label

    try:
        if hasattr(fallback, "guess"):
            fallback_raw = fallback.guess(first)
        else:
            fallback_raw = fallback.get_gender(first)
    except Exception:  # noqa: BLE001
        fallback_raw = None
    return _map_gender_detector(fallback_raw)
...
def _sync_existing_outputs(
    analysis_any_parquet: Path,
    analysis_pos_parquet: Path,
    analysis_any_csv: Path,
    analysis_pos_csv: Path,
    ...
    *,
    ...
    """
    If outputs already exist in either analysis or legacy paths, ensure both locations are populated.
    Returns True when existing artifacts were used (and no rebuild is required).
    """
    source_any = (
        analysis_any_parquet
        if analysis_any_parquet.exists()
        else analysis_any_csv
        if analysis_any_csv.exists()
        else legacy_any_path
        if legacy_any_path.exists()
        else None
    )
    source_pos = (
        analysis_pos_parquet
        if analysis_pos_parquet.exists()
        else analysis_pos_csv
        if analysis_pos_csv.exists()
    ...
    atomic_write_parquet(any_df, analysis_any_parquet)
    atomic_write_parquet(pos_df, analysis_pos_parquet)
    atomic_write_csv(any_df, analysis_any_csv)
    atomic_write_csv(pos_df, analysis_pos_csv)
...
def main(argv: list[str] | None = None) -> int:
    ...
    source_dim_root = paths.source_dir / "dimensions_api"
    analysis_any_parquet = paths.analysis_dir / "outputs_concat_with_any_number_authors.parquet"
    analysis_pos_parquet = paths.analysis_dir / "outputs_concat_with_positive_authors.parquet"
    analysis_any_csv = paths.analysis_dir / "outputs_concat_with_any_number_authors.csv"
    analysis_pos_csv = paths.analysis_dir / "outputs_concat_with_positive_authors.csv"
    ...
    raw_outputs_path = paths.source_dir / "raw_ref_outputs_data.xlsx"
    ...
    input_paths = {
        ...
        "dimensions_raw_root": source_dim_root,
    }
    output_paths = {
        "analysis_any_parquet": analysis_any_parquet,
        "analysis_positive_parquet": analysis_pos_parquet,
        "analysis_any_csv": analysis_any_csv,
        "analysis_positive_csv": analysis_pos_csv,
    ...
    try:
        if not args.force and _sync_existing_outputs(
            ...
            row_counts = {
                "outputs_any_rows": int(len(read_table(analysis_any_parquet))),
                "outputs_positive_rows": int(len(read_table(analysis_pos_parquet))),
            }
            return 0
        ...
        dim_df = None
        try:
            dim_df = _load_dimensions_raw(source_dim_root)
        ...
        if dim_df is None:
            if _sync_existing_outputs(
                ...
                return 0
            raise FileNotFoundError(
                f"No Dimensions raw chunks found under {source_dim_root / 'raw'} and no existing outputs datasets available."
            )
        ...
        atomic_write_parquet(any_authors, analysis_any_parquet)
        atomic_write_parquet(positive_authors, analysis_pos_parquet)
        atomic_write_csv(any_authors, analysis_any_csv)
        atomic_write_csv(positive_authors, analysis_pos_csv)
